...
class PlaywrightBrowserBackendHarness:
    ...
    def provision(self, fixture: MaterializedBrowserFixture) -> PlaywrightBrowserResource:
        if self._closed:
            raise BrowserHarnessError("Playwright Browser backend is closed")
...
def _cookie_for_playwright(
    ...
    if record.host_only:
        if record.path != "/":
            raise BrowserVerificationError(
                "provider foundation only seeds host-only path '/' losslessly; "
                "arbitrary host-only paths require controlled HTTP Set-Cookie support"
            )
        matches = [
        ...
        # Leading dot is a provider seeding syntax used to establish domain scope.
        # Portable stored-domain identity remains the provenance value without it;
        # hostOnly is never inferred from provider presentation syntax.
