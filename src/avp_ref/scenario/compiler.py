...
_REFERENCE = re.compile(
    r"^(?:env|oracle|mcp|simulator|artifact|agent|benchmark)://[^\s]+$"
)
...
class ScenarioCompiler:
    ...
    def compile(
        ...
        parameters, generator_records = self._resolve_parameters(
            source.get("parameters", {}), active, seeds.data
        )
        ...
        if template.get("generators"):
            raise ScenarioValidationError(
                ...
                (
                    CompileDiagnostic(
                        "AVS-SEM-001",
                        "top-level generators require a registered generator stage; use parameter.generator for Alpha 2",
                        "$.generators",
                    ),
                ),
            )
    ...
    def _resolve_parameters(
        ...
        data_seed: int,
    ) -> tuple[dict[str, Any], tuple[GeneratorRecord, ...]]:
        ...
        for name in sorted(declarations):
            ...
            if name in overrides:
                ...
                records.append(
                    GeneratorRecord(
                        parameter=name,
                        generator_type=str(generator_spec.get("type", "")),
                        generator_version=self._generators.version,
                        seed=child_seed,
                    )
                )
