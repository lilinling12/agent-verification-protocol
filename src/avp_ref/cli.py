...
def cmd_conformance() -> None:
    ...
    if not all(result.passed for result in results): raise SystemExit(1)


def cmd_validate(path: str) -> None:
    template = load_scenario(path); validate_template(template)
...
def _parse_override(raw: str) -> tuple[str, Any]:
    if "=" not in raw: raise argparse.ArgumentTypeError("parameter override must use NAME=VALUE")
    name, text = raw.split("=", 1)
    if not name: raise argparse.ArgumentTypeError("parameter override name cannot be empty")
...
def _load_reference_lock(path: str | None) -> StaticReferenceResolver | None:
    ...
    except (OSError, json.JSONDecodeError) as exc: raise SystemExit(f"invalid reference lock file '{source}': {exc}") from exc
    if not isinstance(data, dict): raise SystemExit("reference lock root must be an object keyed by URI")
...
def cmd_compile(path: str, *, seed: int, overrides: list[tuple[str, Any]], output: str | None, strict_references: bool, lock_file: str | None) -> None:
    template = load_scenario(path); resolver = _load_reference_lock(lock_file); compiler = ScenarioCompiler(resolver=resolver) if resolver else ScenarioCompiler()
    ...
    if output:
        target = Path(output); target.parent.mkdir(parents=True, exist_ok=True); target.write_text(payload, encoding="utf-8"); print(json.dumps({"instance_digest": instance.instance_digest, "output": str(target)}, sort_keys=True))
    else: print(payload, end="")


def cmd_serve(port: int) -> None:
    try: import uvicorn
    except ImportError as exc: raise SystemExit("Install with: pip install -e '.[http]'") from exc
...
def main() -> None:
    parser = argparse.ArgumentParser(prog="avp"); sub = parser.add_subparsers(dest="command", required=True); sub.add_parser("demo"); sub.add_parser("conformance")
    benchmark = sub.add_parser("benchmark"); benchmark.add_argument("--runs", type=int, default=4)
    validate = sub.add_parser("validate", help="validate an AVS ScenarioTemplate"); validate.add_argument("scenario")
    compile_parser = sub.add_parser("compile", help="compile AVS to an immutable ScenarioInstance"); compile_parser.add_argument("scenario"); compile_parser.add_argument("--seed", type=int, default=0); compile_parser.add_argument("--set", dest="overrides", action="append", type=_parse_override, default=[], metavar="NAME=VALUE"); compile_parser.add_argument("--out", dest="output"); compile_parser.add_argument("--strict-refs", action="store_true"); compile_parser.add_argument("--lock", dest="lock_file")
    serve = sub.add_parser("serve"); serve.add_argument("--port", type=int, default=8790); args = parser.parse_args()
    try:
    ...
    except ScenarioCompileError as exc: _print_compile_error(exc); raise SystemExit(2) from exc
