...
| 05 | `05-unified-trace-schema.md` | **SPLIT** | `PROMOTED` — AVP evidence/event identity needed for verification<br>`PROMOTED` — trace correlation, propagation, outcome preservation and telemetry completeness<br>`REJECTED` — AVP-owned universal trace wire schema |
...
- **05 `05-unified-trace-schema.md` — SPLIT:** AVP no longer owns a unified tracing wire format. AVP-specific evidence identity remains AVP authority while trace propagation and telemetry representation remain W3C/OpenTelemetry authority with an AVP mapping profile.
