...
4. required/optional participation semantics;
...
7. real-operation conformance for capability claims.
...
| Released/stale handles fail closed | REUSE | `AVP-ENVIRONMENT-011` | Applies to Fabric/resource references as applicable. |
...
| Resource capability declaration is conformance-bearing | NEW | Base Fabric spec | A declared resource capability activates its normative requirements; backend/product name is insufficient. |
...
| Fabric manifest binds Environment identity, Scenario identity, resource membership, kinds, capabilities, and immutable identity references | NEW | Base Fabric spec + future schema | Define language-neutral representation; do not promote Python `EpisodeManifest`. |
| Required and optional resource participation are explicit | NEW | Base Fabric spec | Prevent a reference implementation from silently omitting required resources. |
...
| Composite snapshot/restore/reset does not imply atomic distributed transaction | NEW | Base Fabric spec | Atomicity may only be claimed through a separately defined capability with conformance evidence. |
...
| Giant universal adapter with optional methods / scattered `supports_*` | NON-NORMATIVE / REJECT | Architecture guardrail | Use resource composition and capability contracts instead. |
...
A **Resource Capability Declaration** is a machine-readable claim that a specific Environment Resource supports a named Resource Capability under the selected profile/revision.

The declaration is conformance-bearing. It is neither a feature flag nor evidence by itself.

A future schema should bind enough profile/revision identity to prevent semantic drift under a stable capability identifier. The exact versioning shape is deferred to the normative/schema design rather than inferred from Python class versions.
...
A resource whose presence and applicable capabilities are prerequisites for the selected Scenario/Fabric profile. A missing or incompatible Required Resource prevents the Fabric from satisfying the requested execution contract.

### Optional Resource

A resource not required by the selected execution contract. Its mere availability must not silently make its capabilities mandatory or alter Scenario semantics.
...
A resource's declared role in a composite operation. The base contract needs to distinguish at least required participation, permitted non-participation, and operation failure. Exact serialized values belong in the schema design.
...
3. **FABRIC-003 — Resource capability honesty**  
   A declared Resource Capability activates the selected capability requirements; unsupported required capability negotiation fails before the requested side effect.
...
5. **FABRIC-005 — Fabric manifest binding**  
   The composition manifest binds Environment/Scenario identity, resource membership, portable kinds/capabilities, and required immutable identity references.

6. **FABRIC-006 — Required resource completeness**  
   The Fabric cannot become ready for a selected execution contract while a Required Resource/capability is absent or incompatible.
...
10. **FABRIC-010 — No implicit atomicity**  
    Composite operation success does not imply cross-resource atomicity; stronger atomicity requires separately specified and conformance-tested semantics.

11. **FABRIC-011 — Evidence/security composition**  
    Fabric descriptors/results inherit existing AVP visibility, secret, Artifact integrity, and SecurityAssurance rules rather than redefining them.
...
- required-resource/capability compatibility failure;
...
**READY FOR NORMATIVE SPEC DRAFTING — NOT READY FOR BACKEND IMPLEMENTATION.**

The base Fabric's new portable surface is sufficiently bounded to draft a small language-neutral specification without relying on a PostgreSQL, Playwright, Linux, OCI, or microVM implementation as authority.

The next authority-chain step is the Environment Fabric normative specification plus requirement index. Schema and executable TCK follow that specification; reference/backend implementation remains downstream.
