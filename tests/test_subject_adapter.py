...
class _AgentHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if step == 1:
            response = {
                "status": "tool_call",
                "call": {"call_id": "c1", "name": "order.get", "arguments": {"order_id": "ord_1"}},
            }
        elif step == 2:
            self.server.saw_previous_result = previous is not None and previous.get("call_id") == "c1"
            response = {
                "status": "tool_call",
                "call": {"call_id": "c2", "name": "refund.create", "arguments": {"order_id": "ord_1"}},
            }
        else:
...
class SubjectAdapterTest(unittest.TestCase):
    ...
    def test_http_adapter_executes_external_step_protocol_and_propagates_trace(self):
        ...
        try:
            ...
            self.assertTrue(all(value and value.startswith("00-") for value in server.traceparents))
        finally:
            server.shutdown()
            server.server_close()
    ...
    def test_user_cannot_spoof_trace_headers(self):
        with self.assertRaises(ValueError):
            HTTPSubjectAdapter("http://127.0.0.1:1", headers={"TraceParent": "00-deadbeef"})
