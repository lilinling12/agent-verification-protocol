...
class MCPGatewayTest(unittest.TestCase):
    ...
    def test_open_discovers_paginated_catalog(self):
        d=self.make_gateway(); desc=d.open(); self.assertEqual("2026-07-28",desc.protocol_version); self.assertTrue(desc.baseline_catalog_digest.startswith("sha256:"))
    ...
    def test_invalid_x_mcp_header_tool_is_excluded(self):
        class Bad(FakeTransport):
            def request(self,method,params=None,*,name=None,extra_headers=None):
                if method=="server/discover": return super().request(method,params,name=name,extra_headers=extra_headers)
                if method=="tools/list": return {"tools":[{"name":"order.get","inputSchema":{"type":"object","properties":{"x":{"type":"number","x-mcp-header":"Bad Header"}}}}],"ttlMs":0,"cacheScope":"private"}
                return super().request(method,params,name=name,extra_headers=extra_headers)
        with self.assertRaises(MCPPermissionDenied): MCPVerificationGateway(Bad(),MCPGatewayPolicy(frozenset({"order.get"})),endpoint_identity="x").open()
    def test_runtime_routes_mcp_and_emits_digests(self):
        def subject(session,task): session.call_tool("order.get",{"order_id":"ord_1"}); return "done"
        g=MCPVerificationGateway(FakeTransport(),MCPGatewayPolicy(frozenset({"order.get"})),endpoint_identity="x")
        rt=ReferenceRuntime(); ep=rt.create_episode(reference_scenario(),reference_agent_system("mcp-probe"),reference_environment(),InProcessSubjectAdapter(subject),g); rt.provision(ep.episode_id); rt.run_subject(ep.episode_id)
        events=[e for e in ep.events if e.event_type=="tool.result" and e.payload.get("protocol")=="mcp"]; self.assertEqual(1,len(events)); self.assertTrue(events[0].payload["schema_digest"].startswith("sha256:")); self.assertTrue(ep.manifest.mcp_gateway_config_digest.startswith("sha256:"))
...
class HTTPMCPTransportTest(unittest.TestCase):
    def setUp(self):
        ...
        class Handler(BaseHTTPRequestHandler):
            def do_POST(self):
                ...
                body=json.dumps({"jsonrpc":"2.0","id":p["id"],"result":r}).encode()
    ...
    def test_json_sse_headers_and_meta(self):
        g=MCPVerificationGateway(HTTPMCPTransport(self.endpoint,timeout_seconds=2),MCPGatewayPolicy(frozenset({"order.get"})),endpoint_identity=self.endpoint); g.open(); g.call_tool("order.get",{"order_id":"ord_1"})
        for h,p in self.requests:
            self.assertEqual("2026-07-28",h["mcp-protocol-version"]); self.assertEqual(p["method"],h["mcp-method"]); self.assertEqual("2026-07-28",p["params"]["_meta"]["io.modelcontextprotocol/protocolVersion"])
        call=[h for h,p in self.requests if p["method"]=="tools/call"][0]; self.assertEqual("order.get",call["mcp-name"]); self.assertEqual("ord_1",call["mcp-param-order-id"])
