...
- [x] close provider-neutral NPR-011 Evidence Plan / Result / Fixture / Comparator detailed design
- [x] complete terminating/intercepting NPR-011 evidence-lab implementation-readiness audit
- [x] produce terminating/intercepting-class NPR-011 acceptance evidence against the reviewed architecture
- [x] produce non-terminating packet-path NPR-011 acceptance evidence against the same portable predicates
- [x] execute and retain cross-mechanism NPR-011 portability acceptance evidence
...
PR #140 then adopted the reviewed non-normative NPR-011 Evidence Plan / Result / Fixture / Comparator detailed-design contract and traceability matrix into `main` at exact commit `0e1485bb494879592eb9956cd2df90cdd24b4325`. The design fixes stable project-evidence responsibilities—including sealed plan materialization, fresh attempt/challenge identity, deterministic exact-byte fixture behavior, independent transport-initiation evidence, provider-neutral comparison, content-addressed retained evidence, reassessment/rerun boundaries, cleanup/noninterference, and failure/Validity separation—without making its illustrative field names, SHA-256 project baseline, witness technology, mechanism APIs, or assessment categories normative protocol surfaces. Exact-main CI #855 (`33721541283`), Relational Parity #248 (`33721541285`), and Browser Reference #121 (`33721541310`) all passed.

PR #142 then adopted `docs/acceptance/alpha3-network-control-terminating-evidence-lab-implementation-readiness.md` into `main` at exact commit `4e14f863ac73943273fe0c13dcbfff2720167de9`. The readiness audit confirms that the terminating/intercepting class can proceed without weakening AEP-0012, but it explicitly blocks Toxiproxy-wrapper-first or generic provider-abstraction work. It requires the first implementation slice to establish the provider-neutral evidence core, deterministic exact-byte fixture, independent front-side and upstream transport-initiation witness, retransmission-vs-retry normalization, fail-closed capture-integrity checks, and content-addressed retained evidence. All three exact-main post-merge check suites completed successfully, including Browser Reference #125; the merge commit is GitHub-verified.

TEL-001, its provider-neutral comparator prerequisites, TEL-002, the separately reviewed privileged evidence lane, and the terminating transport corrections are now main-adopted. PR #156 then adopted the independently reviewed TEL-003 terminating/intercepting evidence record into `main` at exact commit `65ecadcfedc907c4d511cb6af9ac747a57414606`. That record binds trusted-main run `33846543402` and artifact `9926819468`; independent review verified the artifact digest, 452/452 manifest coverage, all content-addressed result references, a positive `SATISFIED` assessment, and rejection of every required faulty assembly by the unchanged provider-neutral comparator. Exact-main CI #909 (`33859905672`), Relational Parity #302 (`33859905673`), and Browser Reference #175 (`33859905696`) all passed on the adoption commit.

PTL-001 through PTL-003 and the required packet-path transport/publication corrections are now main-adopted. PR #167 adopted the independently reviewed PTL-003 non-terminating packet-path evidence record into `main` at exact commit `cabc527d526c5f3cb201e8b03f38a0de3598e31f`. That record binds trusted-main run `34028504186` and artifact `9987837192`; independent review verified the downloaded artifact digest, 361/361 manifest coverage, all 340 content-addressed evidence blobs, all 27 critical result references across the nine-case matrix, a positive `SATISFIED` assessment, and fail-closed rejection of every required faulty assembly by the unchanged provider-neutral comparator. Exact-main CI #971 (`34036453660`), Relational Parity #364 (`34036453728`), and Browser Reference #237 (`34036453688`) all passed on the adoption commit. The non-terminating packet-path acceptance-evidence roadmap item is therefore complete on `main`.

PR #170 then adopted the retained NPR-011 cross-mechanism portability assessment into `main` at exact commit `670dafacbabdba05bd551a8028a1dc216613348f`. `docs/acceptance/alpha3-network-control-cross-mechanism-npr011-acceptance.md` records the assessment over the independently adopted TEL-003 and PTL-003 evidence. The retained reassessment binds both source classes to the same exact AEP-0012 Git blob `0b5ebdddb45533cf91a7bc264dcfe7548b54391c`, reassesses all 19 retained cases with the current topology-aware provider-neutral comparator blob `39169f40083e204560606cd9db0d8f1a1dda57bd`, preserves both terminating hidden-retry witness boundaries, and records the deterministic derived archive at `docs/acceptance/evidence/alpha3-network-control-cross-mechanism-npr011-v0.1.json.gz.b64` with archive SHA-256 `30f2da733781f12827ee49f0ea1aace56ba2992a20a017cb806e66af3a50f079` and decoded JSON SHA-256 `b768f6f6d96327cffa550c0f6db1f0b35ff48696362e4113766cce7195ab28ac`. Exact-main CI #977 (`34079764287`), Relational Parity #370 (`34079764305`), and Browser Reference #243 (`34079764236`) all passed on the adoption commit. The cross-mechanism portability-acceptance evidence roadmap item is therefore complete on `main`.

The next legal Network Control protocol Work Unit is **acceptance-oriented exact-head protocol re-review**. It must review the current AEP-0012 candidate against the now-main-adopted NPR-011 evidence and the existing AEP-0009 / Environment / Core / Scenario / Security / Evidence authority chain, fail closed on any remaining semantic or evidence blocker, and must not treat the selected Toxiproxy or Linux netns/veth/nftables mechanisms as protocol authority.

The completed cross-mechanism evidence closes that evidence prerequisite only. The acceptance-oriented exact-head protocol re-review remains incomplete, and AEP-0012 remains **Proposed** unless and until that re-review is review-closed and the protocol maintainer separately and explicitly authorizes `Proposed → Accepted`.
