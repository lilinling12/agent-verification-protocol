...
The next legal NPR-011 evidence Work Unit is the materially independent **non-terminating packet-path** class against the same portable predicates. It must preserve the reviewed cross-mechanism architecture: mechanism-specific Linux network-namespace/veth/nftables control may implement the packet path, but portable Evidence Plan / Fixture / Result / Comparator responsibilities remain shared only at the evidence-contract level. The packet-path work must not introduce a generic `NetworkBackend`, provider registry, provider SPI/plugin system, or allow privileged control authority to enter the Subject boundary.

NPR-011 cross-mechanism acceptance remains incomplete until the packet-path class has produced coherent retained evidence and the terminating plus packet-path evidence can be assessed together against the same provider-neutral comparator. Only after that cross-mechanism evidence is coherent may acceptance-oriented exact-head protocol re-review close. AEP-0012 remains **Proposed** unless and until the protocol maintainer separately and explicitly authorizes `Proposed → Accepted`.
