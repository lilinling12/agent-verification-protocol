...
The next legal NPR-011 evidence Work Unit is **cross-mechanism portability acceptance** over the two independently main-adopted evidence classes against the same provider-neutral comparator. It must assess the terminating/intercepting and non-terminating packet-path evidence together without introducing mechanism-name branches into portable verdict logic, promoting Toxiproxy or Linux netns/veth/nftables details into protocol authority, or weakening Subject/Evaluator/privileged-Control separation.

NPR-011 cross-mechanism acceptance remains incomplete until the terminating plus packet-path evidence are assessed together and the resulting cross-mechanism evidence is retained coherently. Only after that cross-mechanism evidence is coherent may acceptance-oriented exact-head protocol re-review close. AEP-0012 remains **Proposed** unless and until the protocol maintainer separately and explicitly authorizes `Proposed → Accepted`.
