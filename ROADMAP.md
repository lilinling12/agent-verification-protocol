...
The next legal implementation work unit is **TEL-001**: terminating-lab evidence core + deterministic fixture + independent transport-initiation witness. TEL-001 must prove that raw TCP retransmission is not miscounted as a second initiation while real retry/reconnect/alternate-path attempts are independently detectable, including failed or unaccepted extra attempts. It must fail closed on witness ambiguity, drop/overflow, late arming, interface drift, or an unobserved escape path. TEL-001 must not introduce Toxiproxy control logic, a generic `NetworkBackend`, provider registry, plugin system, or privileged GitHub Actions workflow.

Only after TEL-001 is review-closed and main-adopted may **TEL-002** bind the pinned Toxiproxy terminating mechanism and execute selected/control-path cut, settlement, recovery, target-isolation, and hidden-retry negatives against the same portable predicates. **TEL-003** is then the distinct terminating-class evidence execution/adoption record. The packet-path lab remains a separate later consumer, and NPR-011 cross-mechanism acceptance remains incomplete until both mechanism classes have produced coherent retained evidence against the same provider-neutral comparator. Only after that actual evidence is coherent may acceptance-oriented exact-head protocol re-review close. AEP-0012 remains Proposed throughout this work unless and until the protocol maintainer separately and explicitly authorizes `Proposed → Accepted`.
